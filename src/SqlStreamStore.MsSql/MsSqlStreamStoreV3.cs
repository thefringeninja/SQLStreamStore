...
public sealed partial class MsSqlStreamStoreV3 : StreamStoreBase {
	...
	public MsSqlStreamStoreV3(MsSqlStreamStoreV3Settings settings)
		...
		{
			new("StreamVersion", SqlDbType.Int, true, false, SortOrder.Unspecified, -1),
			new("Id", SqlDbType.UniqueIdentifier),
			new("Created", SqlDbType.DateTime, true, false, SortOrder.Unspecified, -1),
			new("Type", SqlDbType.NVarChar, 128),
			new("JsonData", SqlDbType.NVarChar, SqlMetaData.Max),
			new("JsonMetadata", SqlDbType.NVarChar, SqlMetaData.Max)
		};
	...
	public async Task Migrate(IProgress<MigrateProgress> progress, CancellationToken cancellationToken) {
		...
		Logger.LogInformation("Starting migration from schema v2 to v3...");

		try {
			var checkSchemaResult = await CheckSchema(cancellationToken);
			if (checkSchemaResult.IsMatch()) {
				Logger.LogInformation("Nothing to do, schema is already v3.");
				return;
			}
			...
			Logger.LogInformation("Schema migrated. Starting data migration. Loading stream Ids...");
			...
			// Migrate data
			Logger.LogInformation("{count} streams to be processed...", streamIds.Count);
			int i = 0;
			foreach (var streamId in streamIds) {
				var metadata = await GetStreamMetadataInternal(streamId, cancellationToken);
				if (metadata != null) {
					Logger.LogInformation("Migrating stream {streamId} ({current}/{total}", streamId, i, streamIds.Count);
