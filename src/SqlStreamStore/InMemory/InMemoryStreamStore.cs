...
public sealed class InMemoryStreamStore : StreamStoreBase {
	private readonly InMemoryAllStream _allStream = new();
	private readonly GetUtcNow _getUtcNow;
	private readonly ReaderWriterLockSlim _lock = new(LockRecursionPolicy.SupportsRecursion);
	private readonly Action _onStreamAppended;
	private readonly Dictionary<string, InMemoryStream> _streams = new();
	private readonly Subject<Unit> _subscriptions = new();
	private readonly InterlockedBoolean _signallingToSubscribers = new();
	...
	public InMemoryStreamStore(GetUtcNow? getUtcNow = null, ILogger<InMemoryStreamStore>? logger = null)
		: base(TimeSpan.FromMinutes(1), 10000, getUtcNow, logger: logger) {
	...
	private string? ReadMessageData(string streamId, Guid messageId) {
		using (_lock.UseReadLock()) {
			return !_streams.TryGetValue(streamId, out var stream) ? null : stream.GetMessageData(messageId);
		}
	}
