...
public abstract class ReadonlyStreamStoreBase : IReadonlyStreamStore {
	...
	protected ReadonlyStreamStoreBase(
		...
		GetUtcNow? getUtcNow,
		ILogger? logger = null) : this(getUtcNow, false, logger) {
		_metadataMaxAgeCache = new MetadataMaxAgeCache(this, metadataMaxAgeCacheExpiry,
			metadataMaxAgeCacheMaxSize, GetUtcNow);
	}

	protected ReadonlyStreamStoreBase(GetUtcNow? getUtcNow, bool disableMetaCache = true, ILogger? logger = null) {
		GetUtcNow = getUtcNow ?? SystemClock.GetUtcNow;
		Logger = logger ?? NullLogger.Instance;

		_disableMetadataCache = disableMetaCache;
	}
	...
	public async Task<ReadAllPage> ReadAllBackwards(
		...
		if (Logger.IsEnabled(LogLevel.Debug)) {
			Logger.LogDebug("ReadAllBackwards from position {fromPositionInclusive} with max count {maxCount}.",
	...
	public async Task<ReadStreamPage> ReadStreamForwards(
		...
		CancellationToken cancellationToken = default) {
		ArgumentOutOfRangeException.ThrowIfLessThan(fromVersionInclusive, 0);
		ArgumentOutOfRangeException.ThrowIfLessThan(maxCount, 1);
	...
	public async Task<ReadStreamPage> ReadStreamBackwards(
		...
		bool prefetchJsonData = true,
		CancellationToken cancellationToken = default) {
		ArgumentOutOfRangeException.ThrowIfLessThan(fromVersionInclusive, -1);
		ArgumentOutOfRangeException.ThrowIfLessThan(maxCount, 1);
	...
	public IStreamSubscription SubscribeToStream(
		...
		ArgumentNullException.ThrowIfNull(streamMessageReceived);
	...
	private async Task<ReadAllPage> ReloadAfterDelay(
		...
		if (Logger.IsEnabled(LogLevel.Information)) {
			Logger.LogInformation(
				"ReadAllForwards: gap detected in position, reloading after {DefaultReloadInterval}ms",
