...
public static class SqlStreamStoreHalMiddleware {
	...
	public static IApplicationBuilder UseSqlStreamStoreHal(
		...
		options.LoggerFactory ??= builder.ApplicationServices.GetService<ILoggerFactory>();
		...
		options.LoggerFactory?.CreateLogger(nameof(SqlStreamStoreHalMiddleware))
			.LogInformation("{@index:j}", index);
