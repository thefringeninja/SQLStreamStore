...
internal static class SqlStreamStoreFactory {
	...
	public static Task<IStreamStore> Create(ILoggerFactory loggerFactory, CancellationToken cancellationToken = default) {
		...
		return factory(connectionString, schema, loggerFactory, cancellationToken);
	}
	...
	private static Task<IStreamStore> CreateInMemoryStreamStore(
		...
		ILoggerFactory loggerFactory,
		CancellationToken cancellationToken)
		=> Task.FromResult<IStreamStore>(new InMemoryStreamStore(logger: loggerFactory.CreateLogger<InMemoryStreamStore>()));
...
",
		...
		var settings = new MsSqlStreamStoreV3Settings(connectionString, loggerFactory);
	...
	private static async Task<IStreamStore> CreatePostgresStreamStore(
		...
		using (var connection = new NpgsqlConnection(new NpgsqlConnectionStringBuilder(connectionString) {
			...
			var settings = new PostgresStreamStoreSettings(connectionString, loggerFactory);
