...
public partial class AcceptanceTests {
	...
	public async Task Can_read_streams_forwards_with_prefetch(ReadStreamTheory theory) {
		...
		for (int i = 0; i < streamMessagesPage.Messages.Length; i++) {
			...
			JsonNode.DeepEquals(
					JsonNode.Parse(jsonData ?? "{}"),
					JsonNode.Parse(expectedJsonData ?? "{}"))
				.ShouldBeTrue();
			JsonNode.DeepEquals(
					JsonNode.Parse(message.JsonMetadata ?? "{}"),
					JsonNode.Parse(expectedMessage.JsonMetadata ?? "{}"))
				.ShouldBeTrue();
	...
	public async Task Can_read_streams_backwards_with_prefetch(ReadStreamTheory theory) {
		...
		for (int i = 0; i < streamMessagesPage.Messages.Length; i++) {
			...
			JsonNode.DeepEquals(
				...
				.ShouldBeTrue();
			JsonNode.DeepEquals(
					JsonNode.Parse(streamMessage.JsonMetadata ?? "{}"),
					JsonNode.Parse(expectedMessage.JsonMetadata ?? "{}"))
				.ShouldBeTrue();
