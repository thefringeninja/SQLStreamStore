...
public class MsSqlStreamStoreV3AcceptanceTests(
	MsSqlStreamStoreV3FixturePool fixturePool,
	ITestOutputHelper testOutputHelper)
	: AcceptanceTests(testOutputHelper), IClassFixture<MsSqlStreamStoreV3FixturePool> {
	...
	public async Task Can_drop_all() {
		using (var fixture = await fixturePool.Get(LoggerFactory, "abc")) {
	...
	public async Task Can_check_schema() {
		using (var fixture = await fixturePool.Get(LoggerFactory)) {
	...
	public async Task When_schema_is_not_created_then_should_be_indicated() {
		using (var fixture = await fixturePool.Get(LoggerFactory, "notcreated")) {
			await fixture.MsSqlStreamStoreV3.DropAll();
	...
	public async Task Time_taken_to_append_and_read_large_message() {
		...
		Logger.LogInformation("Append: {StopwatchElapsed}", stopwatch.Elapsed);
		...
		Logger.LogInformation("Read: {StopwatchElapsed}", stopwatch.Elapsed);
	}
